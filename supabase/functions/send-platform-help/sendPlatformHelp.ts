...
const sendPlatformHelp = async (request: EdgeFunctionRequest) => {
    ...
    request.log(`Entering sendPlatformHelp for user ${user.id} (name: ${user.display_name}) and feedback ${feedback}`);
    ...
    const body = `
        ...
        background-color: #f9f9f9; border: 1px solid #e0e0e0; padding: 20px;">
            ...
            <section style="margin-bottom: 20px;">
                <h2 style="font-size: 24px; color: #333333;">Support request from User "${user.display_name}"</h2>
                <p style="font-size: 16px; color: #555;">User information: <br> Name: ${user.display_name} <br> Email: ${user.email} <br> Uid: ${user.id}</p>
